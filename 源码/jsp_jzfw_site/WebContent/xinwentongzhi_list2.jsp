...
String fysql="select count(id) as ss from xinwentongzhi where leibie='"+lb+"' and tianjiaren='"+request.getSession().getAttribute("username")+"'";
  ...
  String sql="";
  sql="select  * from xinwentongzhi where leibie='"+lb+"' and tianjiaren='"+request.getSession().getAttribute("username")+"'";
...
%>
  ...
  <br>
  <p align="center">以上数据共<%=i %>条&nbsp;共<%=zgs%>条记录&nbsp;&nbsp;<%=page_record %>条/页　<a href="xinwentongzhi_list2.jsp?page=1&lb=<%=lb%>">首页</a>　<a href="xinwentongzhi_list2.jsp?page=<%= curpage-1%>&lb=<%=lb%>">上一页</a>　<A href="xinwentongzhi_list2.jsp?page=<%= curpage+1%>&lb=<%=lb%>">下一页</A>　<a href="xinwentongzhi_list2.jsp?page=<%=zys %>&lb=<%=lb%>">尾页</A>　当前第<FONT color=red><%=curpage %></FONT>页/共<FONT color=red><%=zys %></FONT>页
